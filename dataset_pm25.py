...
import numpy as np
import torch
from llm_utils import attach_llm_fields, pm25_split_tag, resolve_llm_config
...
class PM25_Dataset(Dataset):
    def __init__(
        ...
        self.split_tag = split_tag or pm25_split_tag(mode, validindex)
        ...
        self.feature_names = [str(col) for col in df.columns[:target_dim]]
        df_gt = pd.read_csv(
...
def get_datasets(validindex=0, llm_config=None, target_strategy="mix"):
    dataset = PM25_Dataset(
        mode="train",
        validindex=validindex,
        llm_config=llm_config,
        split_tag=pm25_split_tag("train", validindex),
        target_strategy=target_strategy,
    )
    dataset_valid = PM25_Dataset(
        mode="valid",
        validindex=validindex,
        llm_config=llm_config,
        split_tag=pm25_split_tag("valid", validindex),
        target_strategy=target_strategy,
    )
    dataset_test = PM25_Dataset(
        mode="test",
        validindex=validindex,
        llm_config=llm_config,
        split_tag=pm25_split_tag("test", validindex),
        target_strategy=target_strategy,
    )
    return dataset, dataset_valid, dataset_test


def get_dataloader(batch_size, device, validindex=0, llm_config=None, target_strategy="mix"):
    dataset, dataset_valid, dataset_test = get_datasets(
        validindex=validindex,
        llm_config=llm_config,
        target_strategy=target_strategy,
    )
