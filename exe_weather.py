...
from dataset_weather import get_weather_dataloader
from utils import evaluate
from Mixup增强 import train
from 多时间步和层间main_model import CSDI_Weather


parser = argparse.ArgumentParser(description="PDDM-LLM Weather imputation")
...
parser.add_argument("--data_path", type=str, default="./data/Weather")
...
foldername = f"./save/weather_{current_time}/"
print("model folder:", foldername)
os.makedirs(foldername, exist_ok=True)
with open(os.path.join(foldername, "config.json"), "w", encoding="utf-8") as f:
    json.dump(config, f, indent=4)

train_loader, valid_loader, test_loader, scaler_params = get_weather_dataloader(
...
model = CSDI_Weather(config=config, device=args.device).to(args.device)
...
if isinstance(scaler_params, dict):
    scaler = scaler_params["scale_"]
    mean_scaler = scaler_params["mean_"]
else:
    mean_scaler = scaler_params[0]
    scaler = scaler_params[1]

scaler = torch.as_tensor(scaler, device=args.device).float()
mean_scaler = torch.as_tensor(mean_scaler, device=args.device).float()
